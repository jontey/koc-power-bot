...
Tabs.tower = {
	...
	// NOTE: init is called once from the bot startup code, and should only be called from there ...
    init: function(div){
		var t = Tabs.tower;
        t.myDiv = div;
		t.alertState = {
...
// NOTE: The following line ADDS an event listener to main namespace. When t.init()  is called from toggleAlertState(), another   
//    event listener is added each time.  When the page is unloaded (refreshed), t.onLoad() will be called once for each time 
//    that toggleAlertState() had been called during the session!
		...
		new MyAjaxRequest(unsafeWindow.g_ajaxpath + "ajax/gate.php" + unsafeWindow.g_ajaxsuffix, {
			...
			onSuccess: function (rslt) {
				if (rslt.ok) {
					...
					if (rslt.updateSeed) {  
						unsafeWindow.update_seed(rslt.updateSeed) // not needed, and can cause undesirable results
					}
// NOTE: t.init() should not be called!!  t.init redraws the entire tab, reads from GM_get and does initialitaion that should only be done once.  
//		 If you only need to redraw the "DEF=xxx" button text, there should be a function that does only that :) 
					t.init(t.myDiv);
					unsafeWindow.Modal.hideModal(); // not needed, and can cause undesirable results
				} else {
					var errmsg = unsafeWindow.printLocalError(rslt.error_code || null, rslt.msg || null, rslt.feedback || null);
// Will cause function to terminate due to js error, 'pbTowerError' element does not exist:					
...
// *** NOTE: ***  The following line could be a real problem!  An 'embed' DOM element is added every time a sound is played and 
//   never removed or re-used?  A user who is AFK could have 600 of these added to document.body with just one 
//   incoming march! (1 hour march, every 10 seconds).  
    ...
    toggleAlertState: function(obj){
        ...
        if (t.alertState.running == true) {
            ...
            t.saveAlertState();		// shouldn't be done here, needs only to be done in t.onUnload()
