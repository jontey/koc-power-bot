...
// @description    Automated features for Betas of Camelot
// ==/UserScript==
...
if (document.URL.search(/apps.facebook.com\/betasofcamelot/i) >= 0){
  facebookInstance ();
  return;
}
...
function facebookInstance (){
  function setWide (){
    var iFrame = null;
    var e = document.getElementById('app_content_299415635051');
	if(e){
...
,
  ...
  postToChat : function (m){
    ...
    //if (m.marchType == 4) t.movetroops(m.toCityId);
...
// TODO: actionLog ?  
  function watchdog (){
    try {
//      if (document.getElementById('app_content_299415635051').firstChild.firstChild.childNodes[1].firstChild.tagName!='IFRAME'){
      if (document.getElementById('app_content_299415635051') == null){
        logit ("BOC NOT FOUND!"); 
        KOCnotFound(5*60);
      } 
    } catch (e){
...
function kocWatchdog (){
  ...
  setTimeout (watchdog, INTERVAL);
  function watchdog (){
logit ("BOC WATCHDOG: "+ document.getElementById('mod_maparea'));    
    if (document.getElementById('mod_maparea')==null){
      actionLog ("BOC not loaded");
      KOCnotFound(2*60);
    }     
  }
}
...
function reloadKOC (){
  ...
  var goto = 'http://apps.facebook.com/betasofcamelot/?s='+serverId;
...
**/        
  ...
  fetchGiftsPage : function (notify){
    ...
    GM_AjaxGet ('http://www.facebook.com/?sk=apps&ap=1', '', parseGiftsPage, 'FB Gifts Page');
    ...
    function parseGiftsPage  (p){
      ...
      try {    
        ...
        for (var i=0; i<m.length; i++){
          if ( m[i].indexOf('betasofcamelot')<0)
            continue;
