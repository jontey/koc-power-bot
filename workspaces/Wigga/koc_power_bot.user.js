...
// TODO: Make sure WinPos is visible on-screen ?
  ...
  mainPop = new CPopup ('pb', Options.pbWinPos.x, Options.pbWinPos.y, 785,600, Options.pbWinDrag,
      function (){
        tabManager.hideTab();
...
var m  = '<DIV id=pbRaidHelp><TABLE id=pbraidhelp width=100% height=0% class=pbTab><TR align="right"><TD><INPUT id=RaidHelp type=submit value="HELP"></td></table></div>';  
    m += '<TABLE id=pbRaidAdd width=100% height=0% class=pbTab><TR align="center">';   
	  ...
	  m += '<CENTER><SELECT id=RaidKnights type=list></select></center>';
	  m+= '<BR><CENTER>'+ strButton20('Raid and save', 'id=pbRaidSave') +'</center>';
          
    pop.getMainDiv().innerHTML = m;
    
    t.getKnights();
    document.getElementById ('RaidHelp').addEventListener('click', t.helpPop , false);
  ...
  helpPop : function (){
     ...
     pop.getTopDiv().innerHTML = '<CENTER><B>Power Bot Help: Barb Raids</b></center>';
     pop.show (true);
   },
  			...
  			if (t.rslt['queue'][y]['botMarches'] != undefined) {
  				for (k in Seed.queue_atkp['city' + t.cityId]){
  					if (Seed.queue_atkp['city' + t.cityId][k]['marchId'] == t.rslt['queue'][y]['botMarches']['marchId']) {
  						...
  						//if (botMarchStatus ==3 && returnUnixTime < now) z+='<TD><img src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/returning.jpg></td>';
  						if (botMarchStatus ==3) z+='<TD align=center><img src=http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/autoAttack/raid_stopped_desat.png></td>';
    	...
    	for (y in t.save){
    	 ...
    	 new AjaxRequest2(unsafeWindow.g_ajaxpath + "ajax/_dispatch.php" + unsafeWindow.g_ajaxsuffix, {
    				 ...
    				 onSuccess: function(transport){
    					var rslt = eval("(" + transport.responseText + ")");
    	                  if (rslt.ok) {
								for (y=0;y<t.save.length;y++){
									if (t.save[y] != undefined && t.save[y]['MarchId'] == Id){
										 update = t.save.slice (y);
										 t.save = update;
									}	
								}
								var serverID = getServerId();
								setTimeout (function (){GM_setValue ('SavedRaids_'+serverID, JSON2.stringify(t.save));}, 500);
  ...
  init : function (div){
    ...
    try {      
      m = '<DIV style="height:500px; max-height:500px; overflow-y:auto"><TABLE width=100% class=pbOptions cellspacing=0 cellpadding=0>\
        ...
        <TR><TD><INPUT id=autobuild type=checkbox /></td><TD>Autobuild MM (interval=10sec.).</td></tr>\
...
Tabs.AutoTrain = {
  ...
  myDiv: null,
      ...
      m += '<TD><INPUT id=pbReassShowRoutes type=submit value="Help"></td>';
      m += '</tr></table></div>';
...
function AutoTrain(){
     ...
     if (Food >= TrainOptions['Keep'][(TrainCity+1)]['[Food']) Food = TrainOptions['Keep'][(TrainCity+1)]['[Food'];
     if (Wood >= TrainOptions['Keep'][(TrainCity+1)]['[Wood']) Wood = TrainOptions['Keep'][(TrainCity+1)]['[Wood'];
     if (Stone >= TrainOptions['Keep'][(TrainCity+1)]['[Stone']) Stone = TrainOptions['Keep'][(TrainCity+1)]['[Stone'];
     if (Ore >= TrainOptions['Keep'][(TrainCity+1)]['[Ore']) Ore = TrainOptions['Keep'][(TrainCity+1)]['[Ore'];
     ...
     //logit('Food ' + (Food/cost[1]).toFixed(0)+'/'+ idle);
     if ((Wood/cost[2]) < idle) idle = (Wood/cost[2]).toFixed(0);
     //logit('Wood ' + (Wood/cost[2]).toFixed(0)+'/'+ idle);
     if ((Stone/cost[3]) < idle) idle = (Stone/cost[3]).toFixed(0);
     //logit('Stone ' + (Stone/cost[3]).toFixed(0)+'/'+ idle);
     if ((Ore/cost[4]) < idle) idle = (Ore/cost[4]).toFixed(0);
     idle = idle-1;
     //logit('Ore ' + (Ore/cost[4]).toFixed(0)+'/'+ idle);
     //if (idle < 0) idle = 0;
     //logit('2 '+idle);
     ...
     new AjaxRequest(unsafeWindow.g_ajaxpath + "ajax/train.php" + unsafeWindow.g_ajaxsuffix, {
       ...
       onSuccess: function(transport) {
         ...
         if (rslt.ok) {
           ...
           logit(TrainCity + ': Train ' + idle + ' -  ' + 'TroopType: ' +unitId );
         } else {logit(TrainCity + ': FAIL Train ' + idle + ' -  ' + 'TroopType: ' + unitId + ' - '+ rslt.msg);}
       },
       onFailure: function() {}
     });
     }
