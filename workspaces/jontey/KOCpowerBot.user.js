...
var Options = {
  ...
  pbWideMap    : false,
  pbFoodAlert  : false,
  alertConfig  : {aChat:false, aPrefix:'** I\'m being attacked! **', scouting:false, wilds:false, defend:true, minTroops:10000, spamLimit:10, lastAttack:0, barbautoswitch:false, raidautoswitch: {}, marches: {}, },
...
//    t.mss = new CmatSimpleSound(SWF_PLAYER_URL, null, {height:36, width:340}, t.e_swfLoaded, 'debug=y');
  ...
  saveincomming : function (m){
    var t = Tabs.tower;
	if(!Options.alertConfig.marches['m'+m.marchId]){
		Options.alertConfig.marches['m'+m.marchId] = m;
	}
	saveOptions();
  },
  ...
  checkrecall : function (){
    var t = Tabs.tower;
	var now = unixTime();
	for (var k in Options.alertConfig.marches){   // compare each incoming march with saved marches
		if(Options.alertConfig.marches[k].arrivalTime > now)//Delete saved attacks that have already occured
			delete Options.alertConfig.marches[k];
		if(!Seed.queue_atkinc[k]){
			Options.alertConfig.marches[k].marchStatus = 9;
			t.newIncoming(Options.alertConfig.marches[k]);
			delete Options.alertConfig.marches[k];
		} else if(Seed.queue_atkinc[k].marchStatus == 9)
			t.newIncoming(Seed.queue_atkinc[k]);
	}
  },
...
/********** Guardian tab **********/
/**** _dispatch.php
ctrl=Guardian&action=summon&cityId=65059&type=wood
******/
/****** reply
{"ok":true,"summonFinishTime":1312941458,"summonGuardian":{"cityId":"65059","type":"wood","time":86944,"name":"Guardian","upgrade":false,"cl0":"8","ul0":9,"cl2":"+13% Wood","cl3":"+13% Troop HP","ul2":"+16% Wood","ul3":"+16% Troop HP","required":{"food":"384000","wood":"384000","stone":"384000","ore":"384000","wild":"9"},"youOwn":{"food":7.88053933e+10,"wood":2.01641492e+9,"stone":201481414,"ore":1.39767738e+9,"wild":"8"}},"_ctrl":"Guardian","_action":"summon"}
*****/
/****** _dispatch.php
ctrl=Guardian&action=build&cityId=65059&type=ore&permission=0
***/
/***** reply
{"updateSeed":{"city":{"65059":{"production":{"ok":true,"mightInc":0,"reports":[],"paid":true,"fiscalUnixTime":1.31294169e+9,"happiness":100,"grievance":"0","population":22500,"laborPopulation":"18000","populationCap":"22500","gold":563233435,"goldUpkeep":"1270","taxRate":"0","resourceUnixTime":1.3129417e+9,"resource1x3600":2.83690188e+14,"resource2x3600":7.25908292e+12,"resource3x3600":7.25322293e+11,"resource4x3600":5.03162776e+12,"resource1Productivity":"230000","resource2Productivity":"0","resource3Productivity":"0","resource4Productivity":"0","resource1Capx3600":"82836000000","resource2Capx3600":"40680000","resource3Capx3600":"36000000","resource4Capx3600":"36360000","resource1Upkeep":"38154891","resource2Upkeep":"0","resource3Upkeep":"0","resource4Upkeep":"0"}}}},"time":680,"buildingId":"1919031","initTS":1.3129417e+9,"ticksNeeded":"1800","timeNeeded":680,"_ctrl":"Guardian","_action":"build"}
*****/
...
var AjaxMarch = {
  ...
  init : function (){
	var t = AjaxMarch;
	if(marchqueue.length > 0){
		sendmarch(marchqueue[0].params, marchqueue[0].notify);
		marchqueue.splice(0, 1);
	}
	setTimeout(t.init, 5000);
  },
  
  addqueue : function (march){
  var t = AjaxMarch;
	if(marchqueue.length < t.maxqueue){
		if(marchqueue.length > 0)
			marchqueue[marchqueue.length] = march;
		else
			marchqueue[0] = march;
	}
  },
  
  sendmarch : function (info, notify){
	var params = unsafeWindow.Object.clone(unsafeWindow.g_ajaxparams);
    for(var k in info){
		params[k] = info[k];
	}
	...
	new MyAjaxRequest(unsafeWindow.g_ajaxpath + "ajax/march.php" + unsafeWindow.g_ajaxsuffix, {
		 ...
		 if (rslt.ok) {
			 ...
			 if(rslt.updateSeed){unsafeWindow.update_seed(rslt.updateSeed)};
			 if (notify)
			  setTimeout(function(){ notify(rslt); }, 1000);
		 } else {
			 if (notify)
			  setTimeout(function(){ notify(rslt); }, 1000);
		  }
...
// GM_log(unsafeWindow.Messages.Methods.handleListReports);
// var deletewild = new CalterUwVar('Messages.Methods.handleListReports', [
	// [/<a onclick='modal_messages_reports_chk/im, '<a class=\'button20\' onclick="\'Messages.markCheckedReportsAsRead();return false;\'"><span>Delete Wild/Barb/Transp2</a></span><a onclick=\'modal_messages_reports_chk'],
	// ]);
// deletewild.setEnable(true);
//GM_log(unsafeWindow.Messages.Methods.handleListReports);
GM_log(inspect(unsafeWindow.FB));