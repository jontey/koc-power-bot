...
/****************************  autoBarb Tab by DonDavici  ******************************/
Tabs.autoBarb = {
		tabOrder : 1,                    // order to place tab in top bar
		tabLabel : 'Barbs',            // label to show in main window tabs
		myDiv : null,
		timer : null,  
		//=> ATTACKER VARS START
		cont:null,
		...
		init : function (div){    // called once, upon script startup
			var t = Tabs.autoBarb;
			t.myDiv = div;
			var m = '<DIV class=pbStat>Barb Cities</div><TABLE class=pbTab><TR align=center>';

			for (var i=0; i<Cities.cities.length; i++)
				m += '<TD width=95><SPAN id=barbcity_'+ i +'>' + Cities.cities[i].name + '</span></td>';
			m += '</tr><TR align=center>';
			for (var cityId in Cities.byID)
				m += '<TD><INPUT type=submit id=barbbut_'+ cityId +' value="Barb = OFF"></td>';
			m += '</tr><TR align=center>';

			t.myDiv.innerHTML = m;

			for (var cityId in Cities.byID){
				var but = document.getElementById ('barbbut_'+ cityId);
				addListener (but, cityId);
			}
			function addListener (but, i){
				but.addEventListener ('click', function (){t.g_butToggleBarbMode(i)}, false);
			}
			setInterval (t.eachSecond, 2000); 
		},

		eachSecond : function (){ // DO SOMETHING
			var t = Tabs.autoBarb;
		 ...
		 * Description:
		 */
		h_normalizeCoord : function (coord){
			if ( coord >= 750)
				coord -= 750;
			else if (coord < 0)
				coord += 750;
			return parseInt (coord/5) * 5;
		},
		/**
		 * Description:
		 */
		h_getMarchTroops : function (cityid){
			var marches = [];

			marches.marchUnits = [];
			marches.marchesurnUnits = [];
			marches.resources = [];
			for (i=0; i<13; i++){
				marches.marchUnits[i] = 0;
				marches.marchesurnUnits[i] = 0;
			}
			cityID = 'city'+ cityid;
			for (k in Seed.queue_atkp[cityID]){   // each march
				...
				if (typeof (march) == 'object'){
					for (ii=0; ii<13; ii++){
						marches.marchUnits[ii] += parseInt (march['unit'+ ii +'Count']);
						marches.marchesurnUnits[ii] += parseInt (march['unit'+ ii +'marchesurn']);
					}
				}
			}
			marchesurn marches;
		 ...
		 * Descripton: 
		 */
		h_getKnights : function(){
			my.Search.ks = [];

			var indcit = 0;
			while (indcit < Cities.numCities) {
				var cit = Cities.cities[indcit];
				var rks = [];
				var ldrs = Seed.leaders["city" + cit.id];
				var ks = Seed.knights["city" + cit.id];

				if (DEBUG_TRACE) document.getElementById("srcResults").innerHTML = "GetKnights cit = " + indcit + ", knights = " + ks + "<br/>" + document.getElementById("srcResults").innerHTML;
				var dbg = "";
				var kcnt = 0;
				...
				for (ik in ks) {
					...
					if (k.knightId != ldrs.resourcefulnessKnightId
						...
						if (DEBUG_TRACE) document.getElementById("srcResults").innerHTML = "ADDING KNIGHT, tot = " + rks.length + ", isbarcit = " + cit.isbarbcit + "<br/>" + document.getElementById("srcResults").innerHTML;

						kcnt++;
						if (cit.isbarbcit || cit.isbugcit) {
							dbg += k.knightName + ",";
							rks.push(k);
							if (DEBUG_TRACE) document.getElementById("srcResults").innerHTML = "ADDED KNIGHT, tot = " + rks.length + ", isbarcit = " + cit.isbarbcit + "<br/>" + document.getElementById("srcResults").innerHTML;
						...
						if (cit.iscrestcit) {
							if (lastk == null) {
								...
								if (DEBUG_TRACE) document.getElementById("srcResults").innerHTML = "found knight in city " + indcit + "<br/>" + document.getElementById("srcResults").innerHTML;
								lastk = null;
							}
						}
					}  
				}
				if (DEBUG_TRACE) document.getElementById("srcResults").innerHTML = "found " + kcnt + " knights in city " + indcit + ", " + dbg + "<br/>" + document.getElementById("srcResults").innerHTML;
				my.Search.ks[cit.id] = rks;
				indcit++;
			}
		},
		/**
		 * Description: returns the best free knight
		 */
		h_getFreeKnight : function(cit) {
			var curtime = (new Date()).getTime();
			var bestknight = null;
			if (DEBUG_TRACE) document.getElementById("srcResults").innerHTML = "in getfreeknight, ksarr length = " + my.Search.ks[cit.id].length + "<br/>" + document.getElementById("srcResults").innerHTML;

			for (var i = 0; i < my.Search.ks[cit.id].length; i++) {
				if (DEBUG_TRACE) document.getElementById("srcResults").innerHTML = "in getfreeknight<br/>" + document.getElementById("srcResults").innerHTML;
		...
		TrySendAttack : function (x) {
			...
			if (DEBUG_TRACE) document.getElementById("srcResults").innerHTML = "begin loop, nu cit = " + Cities.numCities + "<br/>" + document.getElementById("srcResults").innerHTML;

			var indcit = 0;
			dela = 10;
			while (indcit < Cities.numCities) {
				if (DEBUG_TRACE) document.getElementById("srcResults").innerHTML = "trying cit = " + indcit + "<br/>" + document.getElementById("srcResults").innerHTML;

				var cit = Cities.cities[indcit];

				//alert("tryin to send attack: " + cit.isbugcit);
				if (DEBUG_TRACE) document.getElementById("srcResults").innerHTML = "numcav is now " + numcav + ", marching: " + (march.marchUnits[8] + march.returnUnits[8]) + "<br/>" + document.getElementById("srcResults").innerHTML;
				...
				var bugels = [];
				for (ind = 0; ind < my.Search.mapDat.length; ind++) {
					var elind = ind; 
					...
					if (((el.isbarbel && cit.isbarbcit) || (el.iscrestel && cit.iscrestcit)) && curtime - el.lastTime > 60 * 60 * 750) {
						//alert("found barb camp " + distance);
						if (DEBUG_TRACE) document.getElementById("srcResults").innerHTML = "in loop" + el + ", " + cit + "<br/>" + document.getElementById("srcResults").innerHTML;

						var dist = distance(el.x, el.y, cit.x, cit.y);

						if (el.tileLevel /** (curtime - lasttime)*/ / dist > bestdist) {
							if (DEBUG_TRACE) document.getElementById("srcResults").innerHTML = "bestel found at (" + el[0] + "," + el[1] + "), lv" + el[4] + ", dist = " + dist + ", lasthit " + (curtime - lasttime) + "<br/>" + document.getElementById("srcResults").innerHTML;
							bestel = el;
							bestelind = ind;
				...
				if (bestel != null) {
					...
					if (DEBUG_TRACE)document.getElementById("srcResults").innerHTML = "bestel found vor city " + indcit + " at " + bestel.x+ "," + bestel.y + ", isbarb = " + bestel.isbarbel + ", iscrest = " + bestel.iscrestel + "<br/>" + document.getElementById("srcResults").innerHTML;

					var bestknight = this.GetFreeKnight(cit);

					if (DEBUG_TRACE)document.getElementById("srcResults").innerHTML = "After GetFreeKnight<br/>" + document.getElementById("srcResults").innerHTML;
					//alert(dbg);
					if (bestknight != null) {
						if (DEBUG_TRACE) document.getElementById("srcResults").innerHTML = "knight " + bestknight.knightName + " found<br/>" + document.getElementById("srcResults").innerHTML;

						var curtime = (new Date()).getTime();
						//alert(curtime + (20 * 1000));
						bestknight.lastActivity = curtime + (100 * 1000); // time out this knight for 100s

						if (DEBUG_TRACE)document.getElementById("srcResults").innerHTML = "trying to sendarmy from city " + indcit + " with knight " + bestknight.knightName + "<br/>" + document.getElementById("srcResults").innerHTML;
						if (DEBUG_TRACE)document.getElementById("srcResults").innerHTML = "found bestel " + bestel.x + "," + bestel.y + ", mssinceprev: " + (curtime - bestel.lastTime) + ", pos = " + bestelind + "<br/>" + document.getElementById("srcResults").innerHTML;
						if (DEBUG_TRACE)document.getElementById("srcResults").innerHTML = "found bestel2 " + bestel.x + "," + bestel.y + ", mssinceprev: " + (curtime - bestel.lastTime) + ", pos = " + bestelind + "<br/>" + document.getElementById("srcResults").innerHTML;
			...
			if (DEBUG_TRACE) document.getElementById("srcResults").innerHTML = "end loop<br/>" + document.getElementById("srcResults").innerHTML;
		},
		/**
		 * Description: 
		 */
...
//		***********************
		 ...
		 * Description:
		 */
		g_butToggleBarbMode : function (cityId){
			var t = Tabs.autoBarb;
			var mode = 1;
