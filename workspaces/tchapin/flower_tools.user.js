...
var Options = {
	...
	alertConfig: {
		aChat: false,
		aPrefix: '*** Automated message: TOWER ALERT! ***',
		scouting: false,
		wilds: false,
		minTroops: 10000,
		spamLimit: 10
...
if (ENABLE_TEST_TAB) Tabs.push(['Test', 'Test']);
...
function eachSecond() {
	if (Options.alertConfig.aChat) checkTower();
...
var messageNav = {
	...
	init: function () {
		alterUwFunction('modal_messages', [
			[/}\s*$/, 'setTimeout(messageNav_hook,0); }']
		]);
		alterUwFunction('modal_messages_send', [
	...
	msgSendHook: function () {
		...
		if (to.toLowerCase() != '<officers>' || getMyAlliance()[0] == 0) return false;
...
my.Search = {
	...
	GetKnights: function () {
		...
		while (indcit < Cities.numCities) {
			...
			for (ik in ks) {
				var k = ks[ik];
				if (k.knightId != ldrs.resourcefulnessKnightId && k.knightId != ldrs.politicsKnightId && k.knightId != ldrs.combatKnightId && k.knightId != ldrs.intelligenceKnightId) //&& k.knightStatus + "" == "1")
				{
	...
	TrySendAttack: function (x) {
		...
		while (iindcit < Cities.numCities) {
			...
			for (ind = 0; ind < my.Search.mapDat.length; ind++) //while (ind < 15)
			{
				...
				if (!my.Search || !my.Search.mapDat || elind > my.Search.mapDat.length) continue;
				...
				if (((el.isbarbel && cit.isbarbcit) || (el.iscrestel && cit.iscrestcit)) && curtime - el.lastTime > 60 * 60 * 750) // && numcavmarch <= numcav)// &&*/ ((el[4] == 2 && numcav >= 2000) || (el[4] == 3 && numcav >= 4500)))
				{
					...
					if (el.tileLevel /** (curtime - lasttime)*/ / dist > bestdist) {
						...
						bestel = el;
						bestelind = ind;
						bestdist = el.tileLevel /** (curtime - lasttime)*/
						/ dist;
						//                    if (Math.random() > 0.8)
						//                        break;
					}
				...
				if (bestknight != null) {
					...
					if (bestel.iscrestel) // && my.Search.searchdone)
					{
						...
						if (false) //bestel.fw)
						{
							...
							if (my.Search.crestlvl == 6) my.Search.doMarch(cit.id, bestknight.knightId, bestel.x, bestel.y, 100, 0, 0, 0, 0, 0, dela, my.Search.marchNotify);
							else my.Search.doMarch(cit.id, bestknight.knightId, bestel.x, bestel.y, 299, 0, 0, 1, 0, 0, dela, my.Search.marchNotify);
						}
					}
					if (bestel.isbugel) {
			...
			iindcit++;
			indcit++;
			if (indcit == Cities.numCities) indcit = 0;
		}
			...
			if (el.tileId + "" == rslt.tileId + "") {
				...
				if (el.city != null && typeof (el.city) != "undefined") {
					...
					if (el.iscrestel) {
						...
						if (my.Search.crestlvl == 6) my.Search.doMarch(el.city.id, bestknight.knightId, bestel.x, bestel.y, 1, 3000, 0, 0, 0, 0, 10, function (rslt) {});
						else my.Search.doMarch(el.city.id, bestknight.knightId, bestel.x, bestel.y, 0, 8000, 0, 800, 0, 0, 10, function (rslt) {});
	...
	doMarch3: function () {
		...
		if (params != null && my.Search.checkTileTime(params.xcoord, params.ycoord)) {
			...
			try {
				makeJSONRequest(unsafeWindow.g_ajaxpath + "ajax/march.php" + unsafeWindow.g_ajaxsuffix, params, function (rslt) {
					try {
	...
	getReports: function (notify) {
		var params = unsafeWindow.Object.clone(unsafeWindow.g_ajaxparams);
		params.pageNo = "";
		makeJSONRequest(unsafeWindow.g_ajaxpath + "ajax/listReports.php" + unsafeWindow.g_ajaxsuffix, params, function (rslt) {
			notify(rslt);
		});
	},
	...
	getReport: function (rid, notify) {
		...
		params.rid = rid;
		params.side = 1;
		makeJSONRequest(unsafeWindow.g_ajaxpath + "ajax/fetchReport.php" + unsafeWindow.g_ajaxsuffix, params, function (rslt) {
			notify(rslt);
		});
	},
	...
	deleteReports: function (arr, notify) {
		...
		params.srids0 = "";
		makeJSONRequest(unsafeWindow.g_ajaxpath + "ajax/deleteCheckedReports.php" + unsafeWindow.g_ajaxsuffix, params, function (rslt) {
			//            notify(rslt);
		});
	},
	...
	MapRequest: function (left, top, width, cb) {
		...
		makeJSONRequest(unsafeWindow.g_ajaxpath + "ajax/fetchMapTiles.php" + unsafeWindow.g_ajaxsuffix, params, function (rslt) {
	...
	SearchAlliance: function (allianceName) {
		...
		makeJSONRequest(unsafeWindow.g_ajaxpath + "ajax/allianceGetSearchResults.php" + unsafeWindow.g_ajaxsuffix, params, function (rslt) {
			// alert("notify SearchAlliance " + rslt);
			for (k in rslt.alliancesMatched) {
				var all = rslt.alliancesMatched[k];
				var dip = '';
				if (all.relation && all.relation == 1) dip = 'Friendly';
				else if (all.relation && all.relation == 2) dip = 'Hostile';

				//  alert(all.allianceName + " " + dip);
				if (dip != "Hostile") continue;
	...
	SearchAllianceMembers: function (allianceId, allianceName) {
		...
		makeJSONRequest(unsafeWindow.g_ajaxpath + "ajax/getUserLeaderboard.php" + unsafeWindow.g_ajaxsuffix, params, function (rslt) {
			...
			for (var i = 0; i < rslt.results.length; i++) {
				...
				for (var c = 0; c < p.cities.length; c++) {
					...
					//     alert("adding newel");
					if (p.might >= my.Search.minmighttotickle) my.Search.mapDat.push(newel);
	...
	AutoHelp: function () {
		...
		if (firsthelp != null) my.Search.lasthelp = firsthelp;
...
/*    http_request = new XMLHttpRequest();
   ...
   /*
	...
	createIframe: function (iframeName, width, height) {
		var iframe;
		if (unsafeWindow.document.createElement && (iframe = unsafeWindow.document.createElement('iframe'))) {
	...
	StartAuto: function () {
		...
		my.Search.mapDat = [];
		my.Search.FindBarbs(cit.x, cit.y, 37, function () {
			alert("notify called");
		});

		//   alert("before trysendattack");

		my.Search.SearchAlliance("Elenium");
		//   my.Search.SearchAlliance("Helms");
		setTimeout(function () {
			my.Search.checkstuffrunning()
		}, 180000);
			...
			if (rslt.userInfo[0].allianceId) {
				...
				if (true) //diplo != "friendly")
				{
					...
					for (i = 0; i < my.unfriendlyalliances.length; i++) {
						if (my.unfriendlyalliances[i] == rslt.userInfo[0].allianceId) okies = false;
					}

					if (okies && rslt.userInfo[0].allianceName != "Elenium") {
	...
	FindBarbs: function (x, y, maxdist, notify) {
		...
		setTimeout(function () {
			my.Search.MapRequest(xxx, yyy, 30, t.mapCallback2)
		}, MAP_DELAY);
	},
	...
	mapCallback2: function (left, top, width, rslt) {
		...
		for (k in map) {
			...
			if (map[k].tileType == 51 && !map[k].tileCityId) { // if barb
				...
				if (map[k].tileType == 10) type = 1;
				else if (map[k].tileType == 11) type = 2;
				else type = (map[k].tileType / 10) + 1;
			} else {
				continue;
			}
		...
		if (t.curX > t.lastX) {
			...
			if (t.curY > t.lastY) {
				...
				if (t.citiessearched < Cities.numCities) {
					...
					t.FindBarbs(cit.x, cit.y, 37, function () {
						alert("notify called");
					});
				} else {
					t.searchdone = true;
		...
		setTimeout(function () {
			my.Search.checkmaprequest(x, y)
		}, Math.floor(120000));
		setTimeout(function () {
			my.Search.MapRequest(x, y, 30, t.mapCallback2)
		}, MAP_DELAY);
	},
	...
	checkmaprequest: function (x, y) {
		if (my.Search.searchdone) return;
		if ((new Date()).getTime() - my.Search.lastmaprq > 60000) {
			...
			setTimeout(function () {
				my.Search.MapRequest(x, y, 30, my.Search.mapCallback2)
			}, MAP_DELAY);
		}
	},
	...
	RemoveDoubles: function () {
		...
		for (var r = 1; r < this.mapDat.length; r++) {
			...
			if (elp[0] != el[0] || elp[1] != el[1]) newmapdat.push(el);
		}
...
function officerId2String(oid) {
	if (oid == null) return '';
	else if (oid == 3) return 'Officer';
	else if (oid == 2) return 'Vice Chance';
	else if (oid == 1) return 'Chancellor';
	return '';
}
...
function AddMainTabLink(text, id, eventListener) {
	...
	window.setTimeout(function () {
		AddMainTabLink(text, eventListener)
	}, 2000);
	return null;
}
