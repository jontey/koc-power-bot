...
logit ('Page 2 url-a: (findBetween)'+ findBetween (page, 'form action=\"', '"'));  

var re = /<form action=\"(.*?)\"/i;
var m = re.exec(page);
logit ('Page 2 url-b: '+ m[0]);        
...
logit ('Page 2 url-c: '+ m[0]);        
      var url = m[1].replace ('&amp;', '&', 'g');
logit ('Page 2 url-d: '+ url); 
// TODO: force lang to en       
      gift.dat.url = url;
...
function findBetween (s, a, b){
  var i = s.indexOf(a);
  if (i<0)
    return null;
  var ii = s.indexOf(b, i+a.length);
  if (ii<0)
    return null;
  return (s.substring(i+a.length, ii-1));
}
