...
Tabs.build = {
    ...
    init: function(div){
		...
		//setTimeout(t.e_autoBuild, 30000); //should be at least 10
...
//					 changecity(cityId);
		...
		setTimeout(t.e_autoBuild, 20000); //should be at least 10
    },

	changecity : function (cityId) {
	var t = Tabs.build
	var city = t.getCityNameById(cityId);
	for (i=0;i<Seed.cities.length;i++) {
		if (Seed.cities[i][1]==city) var cityNum=i;
	}
	cityNum++;
		actionLog("active changing city");
	 unsafeWindow.citysel_click(unsafeWindow.citysel_click(document.getElementById('citysel_'+ (cityNum))));
		actionLog("complete changing city");
		//setTimeout(t.e_autoBuild, 10000); //should be at least 10
		//t.doOne(bQi);
		//actionLog("do one again");
	//	return;
	},
    ...
    doOne : function (bQi){
		...
		var truecityid = parseInt(t.getCurrentCityId());
...
actionLog("do one");
		...
		if (Seed.buildings['city' + currentcityid]["pos" + citpos] != undefined && Seed.buildings['city' + currentcityid]["pos" + citpos][0] != undefined) {	
			...
			if (mode == 'build') {
				...
				//try here
				//if (currentcityid != truecityid) {
				//actionLog("changing cities");
				 //new t.changecity(currentcityid,bQi);
				 //actionLog("12changed city");
				 //return;
				 //actionLog("passed return");
				//}
				
				
				
			  actionLog("on proper city");
			
				
				if (invalid == false) {							
					var params = unsafeWindow.Object.clone(unsafeWindow.g_ajaxparams);
					t.changecity(currentcityid);
					params.cid = currentcityid;
					params.bid = "";
					params.pos = citpos;
					params.lv = curlvl + 1;
