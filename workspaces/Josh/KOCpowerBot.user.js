...
//				13:{'name':"-------",'x':375,'y':375},
  ...
  clickedSearch : function (){
   ...
   if (t.opt.searchType == 1 || t.opt.searchType == 0) {
		...
		m+= '<TR><TD class=xtab align=right>Ally Only:</td><TD class=xtab><INPUT id=pafilAlly type=CHECKBOX '+ (Options.allyOnly?' CHECKED':'') +'\><td></tr>';
  ...
  dispMapTable : function (){
    ...
    for (i=0; i<t.mapDat.length; i++){
      ...
      if (t.opt.searchType==2 && type==7 ) {
        if (!Options.allyOnly || t.mapDat[i][12] == 'a') {
      ...
      if (dist <= t.opt.maxDistance){
	  	  if (t.opt.searchType==2 && type==7) {    // if city search
    			...
    			if (isMisted) {
    				...
    				aD = '';
    				if (Dip.ally && Dip.ally['a'+userInfo[uu].a]) var aD = 'a';
...
function getDiplomacy (aid) {
  if (Seed.allianceDiplomacies == null)
    return 'neutral';
  if (Seed.allianceDiplomacies.ally && Seed.allianceDiplomacies.ally['a'+aid] != null)
    return 'ally';
