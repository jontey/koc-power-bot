...
var Version = '0.8.8';
var UpdateVersion ='20110601b-german';
...
var Options = {
  ...
  pbWideMap    : false,
  pbFoodAlert  : false,
  alertConfig  : {aChat:false, aPrefix:'!!! ACHTUNG = ANGRIFF !!!', scouting:false, wilds:false,  defend:false, minTroops:1, spamLimit:10, lastAttack:0 },
...
function pbStartup (){
  ...
  var styles = '.xtab {padding-right: 5px; border:none; background:none; white-space:nowrap;}\
    ...
    table.pbMainTab tr td   {height:60%; empty-cells:show; padding: 0px 5px 0px 5px;  margin-top:0px; white-space:nowrap; border: 1px solid; border-style: none none solid none; }\
  ...
  window.name = 'PT';
  logit ("* KoC Power Bot - Deutsch v"+ Version +" geladen! - Update Version: "+ UpdateVersion +"");
  readOptions();
  readGlobalOptions ();
  readAttackOptions();
  setCities();
...
// TODO: Make sure WinPos is visible on-screen ?
 ...
 if(!GlobalOptions.pbWideScreen && Options.pbWinPos.x > 700){
  ...
  actionLog ("KoC Power Bot - Deutsch - Version: "+ Version +" geladen! - (KofC version: "+ anticd.getKOCversion() +") - Update Version: "+ UpdateVersion +"");
  //FoodAlerts.init();
...
&nbsp;<INPUT id=pbnum3 type=text size=5 maxlength=5 value="'+ Options.celltext.num3 +'" '+(Options.celltext.provider==0?'DISABLED':'')+'\> senden! </td>\
 ...
 m += '</select></td></tr>\
	  ...
	  <TR><TD align=right>Def Status Anzeigen: &nbsp; </td><TD><INPUT id=pbalertDefend type=checkbox '+ (Options.alertConfig.defend?'CHECKED ':'') +'/></td></tr>\
		...
		<TR><TD>Auto Attack Ausschalten wenn du angegriffen wirst.</td><TD colspan="2"><INPUT id=pbalertbarb type=checkbox '+ (Options.alertConfig.barb?'CHECKED':'') +'/></td></tr>\
			</table></td></tr>\
		<TR><TD><BR></td></tr>\
...
Tabs.build = {
  ...
  lbQ: [],
	...
	m += '<TD>Wie soll gebaut werden ?: <SELECT id="pbBuildType">\
		...
		<OPTION value=max>bis Level 11</option>\
   ...
   setInterval(t.startdeletereports,(120000));
    ...
    var m = '<DIV id=pbTowrtDivF class=pbStat>BARBAREN LAGER ANGRIFF - EINSTELLUNG</div><TABLE id=pbbarbingfunctions width=100% height=0% class=pbTab><TR align="center">';
	 if (AttackOptions.Running == false) {
	       ...
	       updatebotbutton('Bot Angriff = AUS', 'pbbarbtab');
	   } else {
...
t.nextattack = setInterval(t.getnextCity, 1500);
  setInterval(t.startdeletereports,2*60*1000);
     for(i=0;i<Seed.cities.length;i++){
    		...
    		else document.getElementById(element).innerHTML =  '<b>Gefunden</b>: ' + t.barbArray[i+1].length;
    }
...
var troopDef = [
 ['Versorger', 1],
  ...
  t.troopselect = new CPopup ('pbtroopselect', 0, 0, 600, 445, true, function(){t.saveTroops();});
  	 t.troopselect.centerMe (mainPop.getMainDiv());  
	  t.troopselect.getTopDiv().innerHTML = '<CENTER><b> Truppen Einstellung für Server: '+getServerId()+'</b></CENTER>';
 var z= '<DIV id=pbTraderDivD class=pbStat>TRUPPENSTÄRKE</div><TABLE width=100%><TR>';
 	 z+='<TD></td>';
 for(var i=0; i<troopDef.length; i++)
 	 z+='<TD>'+troopDef[i][0]+'</td>';
  	 z+='<TD>Min. Entfernung</td><TD>Max. Entfernung</td>';
 for(i=0;i<10;i++){
 ...
 for(var j=0; j<troopDef.length; j++){
   ...
   barbOptions: function(){
  	 ...
  	 if(t.barboptions == null)	
		t.barboptions = new CPopup ('pbbarboptions', 0,0, 430,380, true);
  	 t.barboptions.centerMe (mainPop.getMainDiv());  
	 t.barboptions.getTopDiv().innerHTML = '<CENTER><b> Angriff Einstellung für Server: '+getServerId()+'</b></CENTER>';
  	var y = '<DIV style="max-height:400px; overflow-y:auto;"><DIV class=pbStat>BARBAREN LAGER</div><TABLE width=100%>';
 y +='<TR><TD style="margin-top:5px; text-align:center;"><INPUT id=pbresetbarbs type=submit value="Zurücksetzten"></td>';
 y +='<TD style="margin-top:5px; text-align:center;"><INPUT id=pbpaintbarbs type=submit value="Anzeigen"></td>';
 y += '<TD><SELECT id=pbcity type=list></td></tr></table>';
 y +='<TD style="margin-top:5px; text-align:center;"><DIV class=pbStat> ANGRIFF - EINSTELLUNG </div></td><TABLE>';
     y +='<TR><TD>Intervall: <INPUT id=pbsendint type=text size=4 maxlength=4 value='+ AttackOptions.SendInterval +' \> Sekunden <font color=#600000><b>(benötigt Refresh)</b></font></td></tr>';
     y +='<TR><TD>Max. Such Entfernung: <INPUT id=pbmaxdist type=text size=4 maxlength=4 value='+ AttackOptions.MaxDistance +' \></td></tr>';
     ...
     y +='<TR><TD><INPUT id=deletetoggle type=checkbox '+(AttackOptions.DeleteMsg?'CHECKED':'')+' /> Barbaren Lager/Transport Berichte Automatisch Löschen!</td></tr>';
     y +='<TR><TD><INPUT id=deletes0toggle type=checkbox '+(AttackOptions.DeleteMsgs0?'CHECKED':'')+' /> Empfangene Transport Berichte Automatisch Löschen!</td></tr>';
	 y +='<TR><TD><span style=\"font-size:10px; color:#555; line-height:18px; \"><font color=#600000><B><u>Hinweis</u></b></font>: Berichte werden im 2 Min Takt gelöscht!</span></td></tr>';
  ...
  showBarbs: function (citynumber,cityname) {
		...
		t.popTradeRoutes.getTopDiv().innerHTML = '<TD><B>Barbaren Lager für: '+cityname+'</td>';
	 ...
	 _addTab: function(citynumber,cityname,queueId,X,Y,dist,level){
	     ...
	     row.insertCell(5).innerHTML = '<a class="button20" id="barbdel_' + queueId + '"><span>Löschen</span></a>';
	 ...
	 var t = Tabs.Barb;
	     var row = document.getElementById('pbBars').insertRow(0);
	     row.vAlign = 'top';
	     row.insertCell(0).innerHTML = "Stadt";
	     row.insertCell(1).innerHTML = "X";
	     row.insertCell(2).innerHTML = "Y";
	     row.insertCell(3).innerHTML = "Entf.";
	     row.insertCell(4).innerHTML = "Level";
	     row.insertCell(5).innerHTML = '<a class="button20" id="barbdelAll"><span>Koordinaten Löschen</span></a>';
  ...
  barbing : function(){
       ...
       document.getElementById(element1).innerHTML = '<b>Gesendet</b>: ' + AttackOptions.BarbsDone[city]; 
       var element2 = 'pddataarray'+(city-1); 
       document.getElementById(element2).innerHTML =  '<b>VP</b> S: ' + slots + ' - L: ' + t.rallypointlevel +' ';
  ...
  doBarb: function(cityID,counter,number,xcoord,ycoord,kid,u1,u9,u6,u7,u8,u10,u12){
  		...
  		document.getElementById('pberror1').innerHTML = '<b>Angriff Versuche</b>: '+ AttackOptions.BarbsTried; 
  		new AjaxRequest(unsafeWindow.g_ajaxpath + "ajax/march.php" + unsafeWindow.g_ajaxsuffix, {
  		         method: "post",
  		         parameters: params,
  		         loading: true,
				 ...
				 document.getElementById(element1).innerHTML = '<b>Gesendet</b>: ' + AttackOptions.BarbsDone[counter]; 
				 ...
				 document.getElementById(element2).innerHTML =  '<b>VP</b> S: ' + slots + ' - L: ' + t.rallypointlevel +'';
  		         ...
  		         document.getElementById('pberror2').innerHTML = '<font color=#600000><b><u>Fehler</u></b></font> <b>VB</b>: ' + AttackOptions.BarbsFailedTraffic; 
  		         document.getElementById('pberror3').innerHTML = '<b>VP</b>: '+ AttackOptions.BarbsFailedRP;
  		         document.getElementById('pberror4').innerHTML = '<b>Ritter</b>:' + AttackOptions.BarbsFailedKnight;
  		         document.getElementById('pberror5').innerHTML = '<b>Unbekannt</b>:' + AttackOptions.BarbsFailedVaria;
  ...
  sendreport: function(){
    ...
    params.subject = "Barbabren Angriff - Stats";
    var message = 'Barbanren Farm Stats: (Gesendet/Insgesamt)' + '%0A';
    for (x=1;x<=Seed.cities.length;x++){	 
    	message+= Seed.cities[x-1][1] + ': (' + AttackOptions.BarbsDone[x] + '/' + t.barbArray[x].length +')' + '%0A';
    }
    message += '%0A'+ 'Verbindungsfehler: ' + AttackOptions.BarbsFailedTraffic +'%0A';
    message += 'Versammlungsplatz Fehler: ' + AttackOptions.BarbsFailedRP +'%0A';
    message += 'Ritter Fehlermeldung: ' + AttackOptions.BarbsFailedKnight +'%0A';
    message += 'Unbekannte Fehler: ' + AttackOptions.BarbsFailedVaria +'%0A';
    message += 'Derzeitig gesendete Angriffe: ' + (AttackOptions.BarbsTried - AttackOptions.BarbsFailedTraffic - AttackOptions.BarbsFailedRP - AttackOptions.BarbsFailedKnight -  AttackOptions.BarbsFailedVaria) +'%0A';
	message += '%0A'+'%0A' + 'Nahrungs Gewinn (für '+AttackOptions.MsgInterval+' Stunde(n) Barbaren Lager Farmen)' +'%0A';
    for (q=1;q<=Seed.cities.length;q++){
    	...
    	message+= Seed.cities[q-1][1] + ': Begin: ' + addCommas(AttackOptions.Foodstatus[q]) + ' - danach:' + addCommas(parseInt(Seed.resources[cityID]['rec1'][0] / 3600)) + ' - Gewinn: ';
    	message += addCommas(gain)  + '%0A';
		total += gain;
    }
	message += '%0A Nahnrugs Gewinn insgesamt: '+addCommas(total)+'%0A';
  ...
  mapCallback : function (left, top, width, rslt){
    ...
    if (t.curX > t.lastX){
      ...
      if (t.curY > t.lastY){
        ...
        document.getElementById(element).innerHTML = '<b>Ziele</b>: ' + t.mapDat.length;
  ...
  init : function (div){
	...
	try {      
	  ...
	  m = '<DIV style="height:500px; max-height:500px; overflow-y:auto"><TABLE width=100% class=pbOptions cellspacing=0 cellpadding=0>\
		<TR><TD colspan=2><u><B>Power Bot - Einstellung</b></u></td></tr>\
...
<TR><TD><INPUT id=pbFoodToggle type=checkbox /></td><TD>Im Allianz Chat Anzeigen wenn die du für weniger als 6 Stunden Nahrung hast <font color=#FF0000>(Check wird jede Stunde durchgeführt!)</font></td></tr>\
		...
		<TR><TD><INPUT id=pbEveryEnable type=checkbox /></td><TD>KoC alle <INPUT id=pbeverymins type=text size=2 maxlength=3 \>\
		  Min. Neu Laden!<font color=#FF0000>(nur wenn die automatischen Angriffe deaktiviert sind!)</font></td></tr>\
	...
	<TR><TD><INPUT id=pbWMapEnable type=checkbox /></td><TD>WideMap <font color=#FF0000>(benötige Wide Screen, die Karten Ansicht wird vergrößert!)</font></td></tr>\
		...
		<BR></table><BR><HR><u>Hinweis</u>: Wenn eine Box nicht klickbar ist hat KoC mal wieder eine änderung vorgenommen!<HR><a href="http://userscripts.org/scripts/show/98414" target="_blank">KoC Power Bot - Deutsch</a>\
		German Version by <a href="http://userscripts.org/users/297645/scripts" target="_blank">PDX</a> <BR>Version: <SPAN class=boldRed> '+ Version +' </span> - Update Version: <SPAN class=boldRed>'+ UpdateVersion +'</span></div>';
	  div.innerHTML = m;
...
Tabs.transport = {
  ...
  t.e_tradeRoutes();
	...
	m += '<TD colspan=4>Transport Check alle <INPUT id=pbtransportinterval type=text size=2 value="'+Options.transportinterval+'"\> Minuten durchführen, wenn erfüllt dann Transportieren!</td></tr></table>';
	  m += '<TD colspan=4>Es müssen mind. Ressourcen für  <INPUT id=pbminwagons type=text size=2 value="'+Options.minwagons+'"\> Wagen vorhanden sein vor den Transport! <font color=#FF0000>(um die Märsche zu reduzieren!)</font></td></tr></table>';
	...
	//m += '<DIV style="margin-top:10px;margin-bottom:5px;"><u>Hinweis</u>:</div>';
	  ...
	  m += '<TD>Menge: <INPUT id=pbtradeamountGold type=text size=11 maxlength=11 value="0"\></td></tr>'
	  m += '</table>';
...
Tabs.Reassign = {
  ...
  count:0,
	...
	t.e_reassignRoutes();
	  ...
	  m += '</tr></table></div>';
	  m += '<DIV id=pbReassignDivD class=pbStat>NEU ZUORDNEN ROUTEN - HINZUGÜGEN</div>';
...
var RefreshEvery  = {
  ...
  doit : function (){
	actionLog ('KoC Power Bot - Deutsch: Refresh ('+ Options.pbEveryMins +' Minuten sind vorbei!)');
	reloadKOC();
  }
}
...
logit ("KOC WATCHDOG: "+ document.getElementById('mod_maparea'));    
	if (document.getElementById('mod_maparea')==null){
	  actionLog ("KoC Power Bot - Deutsch: KoC ist nicht geladen!");
	  KOCnotFound(2*60);
	}     
  }
}
